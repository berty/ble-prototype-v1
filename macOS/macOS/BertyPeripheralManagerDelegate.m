...
- (void)peripheralManager:(CBPeripheralManager *)peripheral didReceiveWriteRequests:(NSArray<CBATTRequest *> *)requests {
    ...
    for (CBATTRequest *request in requests) {
        ...
        if (bDevice == nil) {
            ...
            CBPeripheral *peripherala = [centralManager retrieveConnectedPeripheralsWithServices:@[[BertyUtils sharedUtils].serviceUUID]][0];
            NSLog(@"peripheral: write error unknown peripheral connected %@ state : %lu", [centralManager retrieveConnectedPeripheralsWithServices:@[[BertyUtils sharedUtils].serviceUUID]], peripherala.state);
            [peripherala setDelegate:self.peripheralDelegate];
            BertyDevice *device = [[BertyDevice alloc] initWithPeripheral:peripherala withCentralManager:centralManager];
            [BertyUtils addDevice:device];
            @try {
            [centralManager connectPeripheral:peripherala options:nil];
